...
static void link_key_request(int index, bdaddr_t *dba)
{
	 ...
	 * required */
	if (key_info->type == 0x04 &&
		conn->loc_auth != 0xff && (conn->loc_auth & 0x01) &&
		conn->rem_auth != 0xff && (conn->rem_auth & 0x01))
		hci_send_cmd(dev->sk, OGF_LINK_CTL, OCF_LINK_KEY_NEG_REPLY,
								6, dba);
	else {
...
static void link_key_notify(int index, void *ptr)
{
	...
	if (key_type < 0x03 || (key_type == 0x06 && old_key_type != 0xff) ||
		...
		/* should not store the link key in file */
		err = btd_event_link_key_notify_not_store(&dev->bdaddr, dba);
